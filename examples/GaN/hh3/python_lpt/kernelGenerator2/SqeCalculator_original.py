...
class SqeCalculator:
    ...
    def __init__(self,
        ...
        self._DebyeWallerFactorList1 = None
...
#            print 'eig ', eig
    ...
    def calcSqeCohCreateAllmodes(self, Q, E):
        ...
        # First, we reduce the Qtransfer vector into Brillouin Zone
        qred = self.reduceQvector(qtransfer)
    ...
    def calcSqeCohforExistedPhonons(self, Q):
        """For a given Q-vector, it uses its corresponding energies and polarizationsand to Calculate the coherent S(Q_vector,E), returns as a list 
           all S(Q_vector,E(q_vector)), qvecs and E(q_vector). the difference from calcSqeCohforExistedPhonons are that: 1- No tolerences are included in the  
           calculations, 2- It calculates Debew Waller factor based on Squire, equation (3.72). S(Q,E) calculations is based on Squires, Eq. (3.120)"""
        ...
        #------------------------------
        d=0.0
        self._DebyeWallerFactorList1 = []
        for atomIndex in range(len(self._unitcell.getAtoms())):
            #print self._displacements[atomIndex]
            dd= 0.0
            for j in range(3):
                d= self._displacements[atomIndex][j+1]*qtransfer[j]
                dd=dd+d
            dd=0.5*dd*dd
            self._DebyeWallerFactorList1.append(dd)
            #print "self._DebyeWallerFactorList1", self._DebyeWallerFactorList1
            #print "qtransfer=",qtransfer
        #print self._DebyeWallerFactorList
        #--------------------------------------
        # index the qtransfer
        #-------------------------------------
        l=-1
        for k in self._kpts:
            ...
            if k[0]==qtransfer[0] and k[1]==qtransfer[1] and k[2]==qtransfer[2]:
               ...
               #print "qtransfer=",qtransfer
               #print "k=", k 
               #print "self._kpts=", self._kpts
               #kptIndex = self._kpts.index(qtransfer)
               ...
               for modeIndex in range(self._D*self._numatoms): 
                   ...
                   for atom, atomIndex, pos in zip(self._unitcell.getAtoms(),
                       ...
                       # according to the orientation of the crystal
                       qdotd = N.dot(qtransfer, pos)
                       weight = N.exp(1j * qdotd - self._DebyeWallerFactorList1[atomIndex])
                       weight *= qdote
                       weight *= atom.average_neutron_coh_xs / N.sqrt(atom.mass)
                       innersum += weight
...
#        return self.
    ...
    def readEigenvaluesFromIDFomega2(self, filename='omega2.idf'):
        ...
        for kIndex in range(self._numkpts):
            for modeIndex in range(self._numatoms * self._D):
                energies[kIndex][modeIndex] = THz2meV * om2[kIndex, modeIndex]
    ...
    def reduceQvector(self, q):
        """Brings cartesian reciprocal vector q=(x,y,z) 'into' the reciprocal cell in which phonons are computed,
        i.e.: (x,y,z)->(x',y',z') such that x',y',z' in [0,1( """
	uc = self._unitcell
	ucstar = uc.getReciprocalUnitCell()
	ucstar.setCellVectors(ucstar.getCellVectors() / 2. / N.pi)
	qfrac = ucstar.cartesianToFractional(q)
        qred = ucstar.bringFractionalPositionIntoCenteredCell(qfrac)
	qredcart = ucstar.fractionalToCartesian(qred)
	#qred = N.array(q)
        #assert (len(qred) == 3)
        #for i in range(3):
        #    if qred[i]<0:
